...
# Dry run — estimate cost without making LLM calls
uv run reader-pipeline run --doc aeon-trespass-core --dry-run
```
...
The Gemini CLI gateway uses local cached credentials — no API key needed.
Run `gemini` once manually to set up credentials if not yet configured.
